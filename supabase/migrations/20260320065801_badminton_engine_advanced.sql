-- Migration: Advanced Badminton Engine
-- Extends the BWF-compliant badminton engine with doubles formations, position swapping, and undo functionality.
...
BEGIN
  ...
  ELSIF p_type = 'set_formation' THEN
    IF p_payload->'doubles_positions' IS NOT NULL THEN
      v_state := jsonb_set(v_state, '{doubles_positions}', p_payload->'doubles_positions');
    END IF;
    -- Also handles confirming side for next game
    IF p_payload->>'first_server' IS NOT NULL THEN
      v_state := jsonb_set(v_state, '{server}', to_jsonb(p_payload->>'first_server'));
    END IF;
    v_state := jsonb_set(v_state, '{status}', '"live"');
  ...
  ELSIF p_type = 'point' OR p_type = 'service_fault' THEN
    ...
    -- Keep last 15 elements to avoid infinite growth
    ...
    -- BWF Rule: In doubles, players DO NOT change their respective service courts until they win a point when their side is serving
    -- Meaning only the serving team who wins the point swaps courts.
    IF v_match_type = 'doubles' AND v_point_winner = v_server THEN
      -- The server won the point, so they swap sides with their partner
    ...
    -- Service switches to the point winner
