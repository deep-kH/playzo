-- Cricket Engine V2: Complete rewrite of process_ball and undo_last_ball
-- Adds: strike rotation, bowler clear at over end, innings/match end detection, wicket nullification

-- =============================================
-- 1. REWRITE process_ball
-- =============================================
...
BEGIN
  ...
  -- =============================================
  -- STRIKE ROTATION LOGIC (§4.2)
  -- =============================================
  -- Normal delivery: swap on odd bat runs
  -- Wide: swap if additional runs beyond penalty are odd → (runs_extra - 1) % 2 = 1
  -- No-ball: swap on odd bat runs (penalty is in extras, doesn't cause rotation)
  -- Bye/Leg-bye: swap on odd extra runs (batsmen physically run)
  v_swap_strike := false;
  IF coalesce(p_extra_type, '') = '' THEN
    -- Normal delivery
    v_swap_strike := (l_runs_bat % 2) = 1;
  ELSIF p_extra_type = 'wide' THEN
    -- Wide: penalty=1 in extras, additional runs = runs_extra - 1
    v_swap_strike := ((l_runs_extra - 1) % 2) = 1;
  ELSIF p_extra_type = 'no_ball' THEN
    -- No-ball: penalty=1 in extras, bat runs determine rotation
    v_swap_strike := (l_runs_bat % 2) = 1;
  ELSIF p_extra_type IN ('bye', 'leg_bye') THEN
    -- Byes/leg-byes: batsmen run, extras = runs run
    v_swap_strike := (l_runs_extra % 2) = 1;
  END IF;

  -- Compute new striker/non-striker
  ...
  -- =============================================
  -- OVER TAPE (balls_this_over jsonb for UI)
  -- =============================================
  ...
  -- =============================================
  -- OVER/BALL PROGRESSION + BOWLER CLEAR
  -- =============================================
  IF v_is_legal THEN
    IF v_state.current_ball = 5 THEN
      -- *** OVER COMPLETE ***
      v_over_complete := true;
      -- At end of over, swap strike AGAIN (in addition to any run-based swap)
      -- This effectively double-swaps: if odd runs already swapped, over-swap brings them back
      -- Net effect: end-of-over swap
      DECLARE v_temp uuid;
      BEGIN
      ...
      SET current_over = current_over + 1,
          ...
          -- Clear bowler so frontend prompts for new bowler
      ...
      WHERE match_id = p_match_id;
    ELSE
      -- Normal legal ball, not over-end
      UPDATE public.ls_match_state
    ...
    END IF;
  ELSE
    -- Illegal delivery (wide/no-ball): doesn't advance ball count
    UPDATE public.ls_match_state
  ...
  -- =============================================
  -- WICKET: NULLIFY STRIKER FOR NEW BATTER PROMPT
  -- =============================================
  IF l_is_wicket THEN
  ...
  -- Re-read innings for updated totals
  SELECT * INTO v_innings FROM public.ls_innings WHERE id = v_state.current_innings_id;

  -- =============================================
  -- INNINGS END DETECTION (§5.2)
  -- =============================================
  -- All out: wickets >= players_per_team - 1
  ...
  IF v_innings_ended THEN
    UPDATE public.ls_innings
    SET status = 'completed'::public.innings_status
    WHERE id = v_innings.id;

    IF v_innings.innings_number = 1 AND NOT v_match_ended THEN
      -- End of 1st innings → innings break
      UPDATE public.ls_match_state
      SET last_event = 'innings_ended'
      WHERE match_id = p_match_id;
    ELSE
      -- End of 2nd innings or target chased → match complete
      v_match_ended := true;
    END IF;
  END IF;
  ...
  IF v_match_ended THEN
    ...
    UPDATE public.ls_match_state
    SET last_event = 'match_completed'
    WHERE match_id = p_match_id;
  END IF;
  ...
  -- SNAPSHOTS & STATS
  -- =============================================
  SELECT * INTO v_batting_row FROM public.ls_batting_stats
  WHERE innings_id = v_innings.id AND player_id = v_state.striker_id;

  SELECT * INTO v_bowling_row FROM public.ls_bowling_stats
  WHERE innings_id = v_innings.id AND player_id = v_state.current_bowler_id;
  ...
  v_stats := jsonb_build_object(
    'crr', v_crr, 'rrr', v_rrr, 'projected_score', v_projected_score,
    'partnership_runs', v_state.partnership_runs, 'partnership_balls', v_state.partnership_balls
  );

  UPDATE public.ls_match_state
  SET striker_snapshot = to_jsonb(v_batting_row),
      bowler_snapshot = (to_jsonb(v_bowling_row) || jsonb_build_object(
        'economy', CASE WHEN v_bowling_row.overs > 0
          THEN (v_bowling_row.runs_conceded::numeric / v_bowling_row.overs)::numeric(5,2)
          ELSE 0 END
      )),
      stats = v_stats
  WHERE match_id = p_match_id;
...
-- =============================================
-- 2. REWRITE undo_last_ball (null-safe + restore striker)
-- =============================================
...
BEGIN
  ...
  -- =============================================
  -- The ball record stores exactly who was batting when it was bowled.
  -- Restoring batter_id as striker and non_striker_id as non-striker
  -- also restores the bowler from the ball record.
  UPDATE public.ls_match_state
