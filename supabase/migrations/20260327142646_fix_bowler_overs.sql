-- Fix missing overs tracking for bowlers
...
BEGIN
  ...
  -- Fix: Recalculate bowler overs based on legal deliveries
  ...
  SET striker_snapshot = to_jsonb(v_batting_row),
      bowler_snapshot = (CASE WHEN v_bowling_row IS NULL THEN NULL ELSE to_jsonb(v_bowling_row) || jsonb_build_object('economy', CASE WHEN coalesce(v_bowling_row.overs, 0) > 0 THEN (v_bowling_row.runs_conceded::numeric / v_bowling_row.overs)::numeric(5,2) ELSE 0 END) END),
      stats = v_stats
  WHERE match_id = p_match_id;
END;
$func$;
...
BEGIN
  ...
  IF v_state.last_ball_id IS NULL THEN RETURN; END IF;
  SELECT * INTO v_ball FROM public.ls_balls WHERE id = v_state.last_ball_id FOR UPDATE;
  IF v_ball.id IS NULL THEN UPDATE public.ls_match_state SET last_ball_id = NULL WHERE match_id = p_match_id; RETURN; END IF;
  SELECT * INTO v_innings FROM public.ls_innings WHERE id = v_ball.innings_id FOR UPDATE;
  ...
  IF v_ball.is_wicket THEN
    ...
    WHERE innings_id = v_innings.id AND player_id = coalesce(v_ball.wicket_player_id, v_ball.batter_id);

    UPDATE public.ls_bowling_stats SET wickets = greatest(0, wickets - 1) WHERE innings_id = v_innings.id AND player_id = v_ball.bowler_id;
    UPDATE public.ls_innings SET total_wickets = greatest(0, total_wickets - 1) WHERE id = v_innings.id;
    UPDATE public.ls_innings SET status = 'in_progress'::public.innings_status WHERE id = v_innings.id AND status = 'completed'::public.innings_status;
    UPDATE public.ls_matches SET status = 'live'::public.match_status WHERE id = p_match_id AND status = 'completed'::public.match_status;
  END IF;

  DELETE FROM public.ls_balls WHERE id = v_ball.id;

  -- Fix: recalculate overs from remaining balls
  SELECT count(*) INTO v_bowls FROM public.ls_balls WHERE innings_id = v_innings.id AND bowler_id = v_ball.bowler_id AND is_legal = true;
  ...
  UPDATE public.ls_innings SET total_runs = greatest(0, total_runs - v_total_runs), total_extras = greatest(0, total_extras - v_ball.runs_extra) WHERE id = v_innings.id;
  ...
  WITH last_legal AS (
    SELECT over_number, ball_number FROM public.ls_balls WHERE innings_id = v_innings.id AND is_legal = true ORDER BY sequence DESC LIMIT 1
  )
  ...
  SET current_over = coalesce((SELECT over_number FROM last_legal), 0),
      ...
      score_overs = (coalesce((SELECT over_number FROM last_legal), 0)::numeric + (coalesce((SELECT ball_number FROM last_legal), 0) % 6)::numeric / 10),
      last_event = 'undo'
